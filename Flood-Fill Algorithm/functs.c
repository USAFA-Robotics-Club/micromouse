...
/**
 * @brief This function reads a CSV file that contains information on
 * the wall and position of each and every cell in a CSV maze file.
 * 
 * @param filename The name of the maze file that will be read and used.
 * @return Cell** 
 */
Cell** readFloodFillFile(char filename[]) {

    Cell** mazeWall = (Cell**)malloc(sizeof(Cell*) * 16);
    for (int row = 0; row < 16; row++) {

        mazeWall[row] = (Cell*)malloc(sizeof(Cell) * 16);

        for (int col = 0; col < 16; col++) {
            mazeWall[row][col].floodfillValue = 0;          // with each floodfillValue initialized to 0
            mazeWall[row][col].topWall = FALSE;             // initialize all wall values to be FALSE
            mazeWall[row][col].bottomWall = FALSE;
            mazeWall[row][col].leftWall = FALSE;
            mazeWall[row][col].rightWall = FALSE;
        }
    }
    ...
    while (!feof(file)) {
        fscanf(file, "%d, %d, %d, %d, %d, %d", &row, &col, &top, &right, &bottom, &left);
    
        mazeWall[row][col].topWall = top;             // initialize all wall values to be FALSE
        mazeWall[row][col].bottomWall = bottom;
        mazeWall[row][col].leftWall = left;
        mazeWall[row][col].rightWall = right;
    }
...
void updateMazeValues(Cell** maze, Robot microMouse, Cell** mazeWall) {  
    ...
    maze[row][col].bottomWall = mazeWall[row][col].bottomWall;
...
/**
 * @brief This function uses Curses to display the results/updates of floodfilling
 * a maze and having the micromouse run through and discovering the maze.
 * 
 * @param maze 
 * @param microMouse 
 */
void displayMaze(Cell** maze, Robot microMouse) {
    ...
    attron(A_BOLD);
    // prints the left and top borders of the maze
    move(1, 1);
    printw("+");
    ...
    // this function prints the rest of the maze, along with the floodfill values
    int initRow = 2;
    int initCol = 2;
    move(initCol, initRow);
    for (int row = 0; row < 16; row++) {
        int initCol = 2;
        for (int col = 0; col < 16; col++) {
            //attron(A_DIM);
            printw("%02d ", maze[row][col].floodfillValue);
            //attroff(A_DIM);
            attron(A_BOLD);
            move(initRow + 1, initCol);
            printw("   +");
            move(initRow, initCol + 3);
            printw(" ");
            
            // print known walls here
            if (maze[row][col].topWall == TRUE) {
                move(initRow - 1, initCol);
                printw("---");
            }
    ...
    move((2 * microMouse.robotRow) + 2, (4 * microMouse.robotCol) + 2 + 2);

    attron(COLOR_PAIR(RED_BLACK));
    //attron(A_BLINK);
    attron(A_BOLD);

    if (microMouse.orientation == 0) {
        printw("A");
    }
    if (microMouse.orientation == 1) {
        printw(">");
    }
    if (microMouse.orientation == 2) {
        printw("V");
    }
    if (microMouse.orientation == 3) {
        printw("<");
    }
    ...
    move(3, 86);
    printw("%02d", microMouse.robotRow);
    move(3, 90);
    printw("%02d", microMouse.robotCol);
    move(4, 88);
    if (microMouse.orientation == 0) {printw("NORTH");}
    if (microMouse.orientation == 1) {printw("EAST ");}
    if (microMouse.orientation == 2) {printw("SOUTH");}
    if (microMouse.orientation == 3) {printw("WEST ");}
    move(5, 95);
    printw("%03d", maze[microMouse.robotRow][microMouse.robotCol].floodfillValue);
...
void cursesPrintDisplay() {
    ...
    // Prints the Top Display Box Insides

    attron(A_UNDERLINE);
    move(2, 72);
    printw("MICROMOUSE INFORMATION");
    move(10, 76);
    printw("DISPLAY STATUS");
    attroff(A_UNDERLINE);

    move(3, 69);
    printw("Robot Location: [  ][  ]");
    move(4, 69);
    printw("Robot Orientation: ");
    move(5, 69);
    printw("Current Flood Fill Value: ");
    move(6, 69);
    printw("Total Cells Traveled: ");
